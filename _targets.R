...
list(
  ...
  tar_target(name = raw_data,
             command = "data/mtcars.csv",
             format = "file"),
  tar_target(name = data,
             command = read_csv(raw_data)),
  tar_target(name = clean_data,
             command = data |>
               tibble() |>
               filter(gear %in% c(1,2,3,4))),

  # Analysis
  tar_target(name = plot,
             command = make_plot(clean_data)),
  tar_target(name = analysis,
             command = do_analysis(clean_data)),

  # Presentation
  tar_quarto(name = create_presentation,
             path = "talk-targets.qmd")
)
